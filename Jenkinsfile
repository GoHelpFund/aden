...
// which are part of the Help repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="help-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /help-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "helppay-help/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from helppay-help/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'helppay-help/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from helppay-help/develop"
            }
          }

          if (hasCache) {
            sh "cd /help-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /help-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /help-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /help-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /help-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /help-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /help-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
