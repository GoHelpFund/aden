...
*Setup instructions for a Gitian build of Help Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Help
...
- [Building Help Core](#building-help-core)
...
Clone the git repositories for Help Core and Gitian.
...
in the Help Core repository under 'Fetch and create inputs' to install sources which require
...
To build Help Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Help Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/help/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/gohelpfund/aden
...
For example:
```bash
URL=https://github.com/crowning-/help.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit help=${COMMIT} --url help=${URL} ../help/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit help=${COMMIT} --url help=${URL} ../help/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit help=${COMMIT} --url help=${URL} ../help/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the help git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../help/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/help
SIGPATH=/some/root/path/help-detached-sigs

./bin/gbuild --url help=${BTCPATH},signature=${SIGPATH} ../help/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/help-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/help-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/help-osx-build.assert
```
...
[help/gitian.sigs](https://github.com/helppay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
