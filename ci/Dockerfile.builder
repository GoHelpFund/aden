...
# help_hash
RUN git clone https://github.com/gohelpfund/aden_hash
RUN cd help_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} help
RUN useradd -u ${USER_ID} -g help -s /bin/bash -m -d /help help
...
RUN mkdir /help-src && \
  ...
  chown $USER_ID:$GROUP_ID /help-src && \
