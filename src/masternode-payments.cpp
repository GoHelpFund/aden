...
*
*   Why is this needed?
*   - In Help some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // disable all Help specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::%s -- ERROR: Missing required payment, possible payees: '%s', amount: %f HELP\n", __func__, strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
