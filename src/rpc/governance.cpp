...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 HELP irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on helpcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in help.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("help.conf", statusObj));
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in help.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Help features */
    { "help",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "help",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "help",               "gobject",                &gobject,                true,  {} },
    { "help",               "voteraw",                &voteraw,                true,  {} },
