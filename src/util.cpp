...
const char * const BITCOIN_CONF_FILENAME = "help.conf";
const char * const BITCOIN_PID_FILENAME = "helpd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "help" is a composite category enabling all Help-related debug output
                if(ptrCategory->count(std::string("help"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HelpCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HelpCore
    // Mac: ~/Library/Application Support/HelpCore
    // Unix: ~/.helpcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HelpCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HelpCore";
#else
    // Unix
    return pathRet / ".helpcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty help.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override help.conf
